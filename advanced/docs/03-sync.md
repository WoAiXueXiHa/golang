...
`sync` 包是 Go 并发安全的工具箱：WaitGroup 等协程、Mutex 保护临界区、RWMutex 优化读多写少、Once 保证只执行一次；map 并发要用 Mutex 保护或用 sync.Map；原子操作是轻量级的无阻塞方案适合简单场景——用对工具、记住坑点（不拷贝锁、不成对 Lock/Unlock、不加锁顺序不一致），并发代码才能又安全又高效。
