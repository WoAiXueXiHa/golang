...
示例代码先用 `chan struct{}` 实现同步，再用 `sync.WaitGroup`，实际上展示了两种等协程完成的方案。
...
示例代码里演示了两种死锁，正好对应底层原理文档里死锁两个最重要的场景。
...
示例代码中有两段 atomic demo，演示了原子操作和 Mutex 的不同使用方式：
...
如果两次读到的是同一个值，不能保证中间没被人改过——可能被改成别的、又改回来了。这在指针场景下可能很危险。解决方案是加版本号或者用 tagged pointer（示例代码暂时没涉及，先知道有这个问题就好）。
