...
`defer` 把函数调用延迟到当前函数返回前执行，常用于释放资源、解锁、关闭文件、恢复 panic。

多个 `defer` 按后进先出顺序执行。

`panic` 会沿调用栈向上传播，每一层函数退出前都会执行自己的 `defer`。

`recover` 只能在 `defer` 调用的函数中捕获当前 goroutine 正在传播的 panic。捕获后，panic 停止继续传播。

本质句：`error` 是正常控制流的一部分，`panic` 是异常退出；`defer` 负责退出前收尾，`recover` 只在 defer 里有意义。
...
多个 goroutine 同时修改同一个变量，会产生数据竞争。`sync.Mutex` 是互斥锁，同一时刻只允许一个 goroutine 进入临界区。
...
任务从哪里来：`main` 负责生产任务，把 `Job` 写入 `jobs`。

worker 如何消费任务：每个 worker 都执行 `for job := range jobs`。只要 `jobs` 没关闭，worker 就会继续等任务；`jobs` 关闭且任务读完后，循环结束。

channel 如何连接生产者和消费者：`jobs` 把任务从生产者传给 worker，`results` 把处理结果从 worker 传回主流程。

`WaitGroup` 如何等待所有任务完成：每启动一个 worker 前 `wg.Add(1)`，worker 退出前 `defer wg.Done()`，主 goroutine 用 `wg.Wait()` 等所有 worker 结束。

方向型 channel 让函数意图更清楚：`jobs <-chan Job` 表示 worker 只能接收任务，`results chan<- int` 表示 worker 只能发送结果。
