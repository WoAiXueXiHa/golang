...
* `func RegisterUser(...) (string, error)`：Go 函数原生支持多返回值。大厂规范中，通常将 `error` 作为**最后一个返回值**。
* `return "", errors.New(...)`：当触发业务非法边界时，提前阻断，返回该类型对应的零值（空字符串），并用 `errors.New` 包装错误抛出。
* `if err != nil { ... }`：这行高频代码正是 Go 的 **`if-err` 卫语句哲学**。它要求开发者直面错误，立刻处理，而不是像 Java 一样抛出不确定的 runtime 异常。
...
配合 **【第一部分图1、图2】**：**大厂面试铁律：Go 语言有且只有“值传递”！**
